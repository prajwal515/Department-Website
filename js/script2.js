...
app1.controller('add', ['$scope', '$http', function ($scope, $http) {
 ...
 $scope.checkusn = function(){
  $http({
   method: 'post',
   url: 'check_usn.php',
   data: {usn:$scope.usn}
  }).then(function successCallback(response) {
   $scope.unamestatus1 = response.data;
  });
 }

 // Set class
 $scope.addClass1 = function(unamestatus1){
  if(unamestatus1 == 'Available'){
   return 'response exists';
  }else if(unamestatus1 == 'Not available'){
   return 'response not-exists';
  }else{
